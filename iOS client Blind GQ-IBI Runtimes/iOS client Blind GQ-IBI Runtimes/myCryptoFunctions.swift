...
public func identifyUser (e: BigUInteger, N: BigUInteger, userIdentity: Array<UInt8>, userPrivateKey: Array<UInt8>) -> Bool {
    // Commitment
    let y = generateRandomVal(maxVal: N)
    let Y = y.power(e, modulus: N)
    
    // Challenge
    let c = generateRandomVal(maxVal: N)
    
    // Response
    let z = BigUInteger(Data(userPrivateKey)).power(c, modulus: N).multiplied(by: y).power(1, modulus: N)
